...
fun ObservatoryScreen(
    ...
    val maxCardHeight = screenHeight * 1.0f
    val minCardHeight = 250.dp
    val dynamicHeight = remember(scrollState.value) {
        (minCardHeight.value + scrollState.value / 5).coerceAtMost(maxCardHeight.value)
    }.dp
        ...
        info?.let {
            ...
            Box(
                modifier = Modifier
                    ...
                    .zIndex(2f) // 카드보다 위에
                    .background(Color(0xFF2D1F75)) // 카드와 같은 배경색
            )
        }
